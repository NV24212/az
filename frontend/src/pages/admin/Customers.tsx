...
export default function Customers() {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (editing) {
      updateMutation({ id: editing.customerId, payload });
    } else {
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <button disabled={isCreating || isUpdating} className="px-4 py-2 rounded-lg brand-bg text-white shadow hover:opacity-95 active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed flex items-center gap-2">
              {isCreating || isUpdating ? <Loading /> : (editing ? 'Save Changes' : 'Create Customer')}
            </button>
          </div>
        </form>
      </Modal>

      <ConfirmDialog open={!!confirmDelete} title="Delete Customer" onCancel={() => setConfirmDelete(null)} onConfirm={() => { if (confirmDelete) deleteMutation(confirmDelete.customerId); }} message={<span>Are you sure you want to delete <b>{confirmDelete?.name}</b>?</span>} isPending={isDeleting} />
    </div>
  );
}
